...
class PluginModel(GlancesPluginModel):
    ...
    def __init__(self, args=None, config=None):
        ...
        # Load filesystem descriptions from config
        if config is not None:
            self.fs_descriptions = config.as_dict().get('fs', {})
        else:
    ...
    def msg_curse(self, args=None, max_width=None):
        ...
        # Only process if stats exist and the plugin is enabled
        if not self.stats or self.is_disabled():
            return ret

        # Max width for filesystem name
        if max_width:
            name_max_width = max_width - 13  # Adjust width for display
        else:
            # No max_width defined, return an empty message
            logger.debug(f"No max_width defined for the {self.plugin_name} plugin, it will not be displayed.")
            return ret

        # Header for the FILE SYS section
        msg = '{:{width}}'.format('FILE SYS', width=name_max_width)
        ret.append(self.curse_add_line(msg, "TITLE"))

        # Determine if we are displaying free or used space
        ...
        # Iterate over the filesystem stats, sorted by mount point (or alias if available)
        for i in sorted(self.stats, key=operator.itemgetter(self.get_key())):
            # New line for each filesystem
            ret.append(self.curse_new_line())

            # Get the filesystem name (mount point) and any alias or description
            fs_name = i['mnt_point']
            description = self.fs_descriptions.get(fs_name, '')  # Retrieve description from config

            # Format the filesystem name with the description (if available)
            display_name = f"{fs_name} ({description})" if description else fs_name

            # Ensure the display fits within the width constraints
            if len(display_name) > name_max_width:
                display_name = display_name[:name_max_width] + '_'

            # Add the formatted name to the display
            msg = '{:{width}}'.format(nativestr(display_name), width=name_max_width + 1)
            ret.append(self.curse_add_line(msg))

            # Display either free or used space, based on user argument
            if args.fs_free_space:
            ...
            # Display the total size of the filesystem
    ...
    def msg_curse(self, args=None, max_width=None):
        ...
        for i in sorted(self.stats, key=operator.itemgetter(self.get_key())):
            ...
            # Get the filesystem name (mount point) and any alias or description
            fs_name = i['mnt_point']
            description = self.fs_descriptions.get(fs_name, 'No description found')  # Retrieve description from config
