...
class PluginModel(GlancesPluginModel):
    ...
    def __init__(self, args=None, config=None):
        ...
        # Initialize hide_zero and hide_zero_fields from config or defaults
        if config is not None:
            self.hide_zero = config.get_bool_value(self.plugin_name, 'hide_zero', default=False)
            self.hide_zero_fields = config.get_list_value(
                self.plugin_name, 'hide_zero_fields',
                default=['bytes_recv_rate_per_sec', 'bytes_sent_rate_per_sec']
            )

            # Initialize hide_no_up and hide_no_ip with default values
    ...
    def msg_curse(self, args=None, max_width=None):
        ...
        # Only process if stats exist and the display plugin is enabled
        ...
        # Configure cumulative or bitrate stats
        if args.network_cumul:
            msg = '{:>14}'.format('Rx+Tx' if args.network_sum else 'Rx/Tx')
        ...
        for i in self.sorted_stats():
            # Skip interfaces that are down (issue #765)
            if ('is_up' in i) and (i['is_up'] is False):
                continue

            # Hide stats if all relevant fields are zero
            if all(self.get_views(item=i[self.get_key()], key=f, option='hidden') for f in self.hide_zero_fields):
                continue  # Skip if all fields are zero

            # Format stats
            if_name = i['alias'] if i['alias'] else i['interface_name'].split(':')[0]
            ...
            if args.network_cumul and 'bytes_recv' in i:
                ...
                continue  # Skip if no relevant stats available

            # New line with the interface name
