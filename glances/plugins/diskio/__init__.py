...
class PluginModel(GlancesPluginModel):
    ...
    def __init__(self, args=None, config=None):
        ...
        # Hide stats if they have never been != 0
        if config is not None:
            self.hide_zero = config.get_bool_value(self.plugin_name, 'hide_zero', default=False)

            # Dynamically load the fields to hide from config, or use the default ones
            self.hide_zero_fields = config.get_list_value(
                self.plugin_name, 'hide_zero_fields',
                default=['read_bytes_rate_per_sec', 'write_bytes_rate_per_sec']
            )
        else:
    ...
    def msg_curse(self, args=None, max_width=None):
        ...
        for i in self.sorted_stats():
            # Hide stats if all the relevant fields are zero
            if all(self.get_views(item=i[self.get_key()], key=f, option='hidden') for f in self.hide_zero_fields):
                continue  # Skip if all relevant fields are zero
